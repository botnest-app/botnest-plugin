...
class PluginPackageTests(unittest.TestCase):
    ...
    def test_skill_frontmatter_and_core_safety_rules(self):
        ...
        self.assertIn("## Telegram administration", skill)
        self.assertIn("`telegram_member`", skill)
        self.assertIn("`telegram_chat`", skill)
        self.assertIn("`telegram_message`", skill)
        self.assertIn("`telegram_topic`", skill)
        self.assertIn("`join_request`", skill)
        self.assertIn("Use `kick`", skill)
        self.assertIn("`${message.photo_file_id}`", skill)
...
class McpBridgeTests(unittest.TestCase):
    ...
    def test_bridge_targets_production(self):
        expected_version = load_json(ROOT / "botnest.plugin.json")["version"]
        self.assertEqual(self.proxy.BASE_URL, "https://botnest.app")
        self.assertEqual(self.proxy.MCP_URL, "https://botnest.app/mcp")
        self.assertEqual(self.proxy.PLUGIN_VERSION, expected_version)
        self.assertEqual(
